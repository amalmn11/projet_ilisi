...
//recuperation de reponse saisit
        ...
        if(($result_nbr_reponses['nombre_reponses'] % 5 == 0) && $createur["ETAT_ID"]!=3 && $createur["ETAT_ID"]!=5 )   
        {       
                $req_q = "UPDATE question SET ETAT_ID = 4 WHERE QUESTION_ID = :question_id";//complete
        ...
        elseif(($result_nbr_reponses['nombre_reponses'] < 5) && ($createur["ETAT_ID"]==1))  $req_q = "UPDATE question SET ETAT_ID = 2 WHERE QUESTION_ID = :question_id";//en cours
...
//recuperer l'id de l'utilisateur qui a crée la question
$req_createur_id="SELECT UTILISATEUR_ID FROM question where QUESTION_ID=?";
$stmt_createur_id = $bdd->prepare($req_createur_id);
$stmt_createur_id->execute([$id_question]);
$resultat = $stmt_createur_id->fetch(PDO::FETCH_ASSOC);
$createur_id = $resultat['UTILISATEUR_ID'];
//recuperer les infos de l'utilisateur en cours
$req_createur_reponse="SELECT * FROM utilisateur where UTILISATEUR_ID=?";
$stmt_createur_reponse = $bdd->prepare($req_createur_reponse);
$stmt_createur_reponse->execute([$user_id]);
$resultat_createur_reponse = $stmt_createur_reponse->fetch(PDO::FETCH_ASSOC);

//recuperes les informations de la question
$req_question="SELECT * FROM question where QUESTION_ID=?";
$stmt_req_question = $bdd->prepare($req_question);
$stmt_req_question->execute([$id_question]);
$resultat_req_question = $stmt_req_question->fetch(PDO::FETCH_ASSOC);

if($createur_id != $user_id)
{
        //envoyer notification
$contenu_notification = "Le collaborateur " . $resultat_createur_reponse["NOM"] . " " . $resultat_createur_reponse["PRENOM"] . " a repondu à votre question concernant ".$resultat_req_question['QUESTION_TITRE'];
// Requête d'insertion de la notification
$requete_insert_notification = "INSERT INTO notification (UTILISATEUR_ID, NOTIF_CONTENUE,TYPE) VALUES (:createur_id, :contenu,:typee)";
$stmt_insert_notification = $bdd->prepare($requete_insert_notification);
$stmt_insert_notification->bindParam(':createur_id', $createur_id, PDO::PARAM_INT);
$stmt_insert_notification->bindParam(':contenu', $contenu_notification, PDO::PARAM_STR);
