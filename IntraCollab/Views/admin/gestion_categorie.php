...
<!---------------MODAL------------------->
<div class="modal fade" id="formModifierCategorie" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
...
<script>
        $(document).ready(function () {
            $('.editBtn').on('click', function () {
                $('#formModifierCategorie').modal('show');
                var categorie_id = $(this).closest('tr').find('input[name="cateorie_id"]').val();
                 var categorie_nom = $(this).closest('tr').find('input[name="cateorie_nom"]').val();
                $('#cateorie_id').val(categorie_id);
                $('#cateorie_nom').val(categorie_nom);
            });
        });
</script>
