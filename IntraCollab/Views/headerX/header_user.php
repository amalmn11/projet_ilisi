...
UTILISATEUR_ID = :user_id and EST_LU=0 and TYPE='notification'";
...
//recuperer les messages non lue
$requete_messages_non_lues = "SELECT * FROM notification WHERE 
UTILISATEUR_ID = :user_id and EST_LU=0 and TYPE='message'";
$stmt_messages_non_lues = $bdd->prepare($requete_messages_non_lues);
$stmt_messages_non_lues->bindParam(':user_id', $user_id, PDO::PARAM_INT);
$stmt_messages_non_lues->execute();
$messages_non_lues = $stmt_messages_non_lues->fetchAll(PDO::FETCH_ASSOC);
...
<a class="nav-link nav-icon" href="#" data-bs-toggle="dropdown">
  ...
  <span class="badge bg-success badge-number"><?php echo count($messages_non_lues); ?></span>
</a><!-- End Messages Icon -->

<ul class="dropdown-menu dropdown-menu-end dropdown-menu-arrow notifications">
  <li class="dropdown-header">
    Vous avez  <?php echo count($messages_non_lues); ?> nouveaux messages
    <?php $_SESSION['page']=$_SERVER['REQUEST_URI'];?>
    <a href="../user/marquer_notifications_lues.php"><span class="badge rounded-pill bg-primary p-2 ms-2">lire tous</span></a>
  </li>
  <li>
  ...
  <?php foreach ($messages_non_lues as $message): ?>
    <li class="notification-item">
    <i class="bi bi-check-circle text-success"></i> 
      <div>
        </p><?php echo $message['NOTIF_CONTENUE']; ?></p>
        <p><?php echo $message['DATE_CREATION']; ?></p>
      </div>
    </li>
...
<!-- 
      ...
      <li class="nav-item">
        <a class="nav-link collapsed" href="..\user\feedback.php">
