...
<!-- <div class="modal fade" id="formModifierRoleProjet" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Modifier votre Role dans le projet</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <!-- Formulaire pour ajouter une compétence -->
                    ...
                    <div class="mb-3">
                        ...
                        <div class="mb-3">
                            <label for="newComp" class="form-label">Libelle du Role précédent:</label>
                            <input type="text" name="rolepreced" id="rolepreced" class="form-control"  disabled>
                        </div>
                        <div class="mb-3">
                            <label for="newrole" class="form-label">Nouveau Role:</label>
                            <select id="newrole" name="newrole" class="form-select">
                                    <?php foreach($role as $rol): ?>
                                        <option value="<?php echo $rol["NIVEAU_TITRE"];  ?>">
                                        <?php echo $niveau["NIVEAU_TITRE"];  ?></option>
                                    <?php endforeach; ?>
                            </select>
                        </div> -->
                ...
                <div class="row">
                     ...
                     <?php  foreach($projet as $ligne): ?>
                      <table style="margin:20px;" width="100%">
                        <tr>
                            <td  width="70%">
                                <input type="hidden" name="projet_titre" value="<?php echo $ligne["PROJET_TITRE"]; ?>">
                                <h5 style="color:#700129"><?php  echo $ligne['PROJET_TITRE']; ?></h4>
                            </td>
                            <td>
                                <a href="#" type="button" class="my-btn" data-bs-toggle="modal">
                                    <i class="bi bi-pen-fill"></i>
                                </a>
                                <input type="hidden" name="projet_idd" value="<?php echo $ligne["PROJET_ID"]; ?>">
                                <input type="hidden" name="role_projet_idd" value="<?php echo $ligne["ROLE_PROJET_ID"]; ?>">
                        ...
                        <div id="info">
                          <div  class="formation">
                            ...
                            <b>Role dans le projet : </b><?php echo $ligne['ROLE_PROJET_TITRE']; ?>
                            </div>
        ...
        <div class="card">
            ...
            <form action="..\..\controllers\user\modifier_profile.php" method="POST" enctype="multipart/form-data">
                ...
                <div class="row mb-3">
                    ...
                    <div class="col-md-8 col-lg-9">
                        <input name="newDateNaissance" type="date" class="form-control" id="company" value="<?php echo $infos[0]["DATE_NAISSANCE"]; ?>">
                    </div>
                </div>
                <div class="row mb-3">
                ...
                <div class="col-md-8 col-lg-9">
                    <input name="newAdresse" type="text" class="form-control" id="newAdresse" value="<?php echo $infos[0]["ADRESSE"]; ?>">
                </div>
                </div>
                <div class="row mb-3">
                <label for="fullName" class="col-md-4 col-lg-3 col-form-label">Ville</label>
                <div class="col-md-8 col-lg-9">
                    <input name="newVille" type="text" class="form-control" id="newVille" value="<?php echo $infos[0]["VILLE"]; ?>">
                </div>
                </div>
                <div class="row mb-3">
                <label for="fullName" class="col-md-4 col-lg-3 col-form-label">Code postal</label>
                <div class="col-md-8 col-lg-9">
                    <input name="newCodePostal" type="text" class="form-control" id="newCodePostal" value="<?php echo $infos[0]["CODE_POSTAL"]; ?>">
                </div>
                </div>
                <div class="row mb-3">
                <label for="fullName" class="col-md-4 col-lg-3 col-form-label">Pays</label>
                <div class="col-md-8 col-lg-9">
                    <select name="newPays" id="countrySelect" class="form-select">
                    <option value="<?php echo $infos[0]["PAYS"]; ?>"><?php echo $infos[0]["PAYS"]; ?></option>
                    </select>
                </div>
                </div>
                ...
                <button type="reset" name="" class="btn btn-primary">Annuler</button>
                </div>
            ...
            </div></div></div></div><!--fin edit settings-->
        ...
        $(document).ready(function () {
            $('.editBtn').on('click', function () {
                $('#formModifierRoleProjet').modal('show');
                 var role_id = $(this).closest('tr').find('input[name="role_projet_idd"]').val();
                 var projet_role_titre = $(this).closest('tr').find('input[name="role_projet_titre"]').val();
                 var nivTitre = $(this).closest('tr').find('input[name="competence_nivv"]').val();
                $('#competence_id').val(role_id);
                $('#newComp').val(projet_role_titre);
                $('#newNiv').val(nivTitre);
            });
        });
</script> -->
<!------script pour recuperer les pays------->
<script>
